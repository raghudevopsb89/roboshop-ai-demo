"""Step 3: build the RAG index over the RoboShop SQL data.
...
Note on scope: this index holds per-row facts only -- no pre-computed aggregates.
So it answers "what does ROB007 cost" perfectly, but questions needing arithmetic
or ranking across all rows ("total inventory value", "your most expensive
product") are NOT reliably answerable: pure vector search cannot add or sort, and
top-k retrieval only ever sees k rows. rag-demo-1 solves that by indexing
rollup facts; see its build_index.py if you want to demo that technique here.
"""
import json
import sqlite3
